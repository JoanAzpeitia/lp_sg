...
class SceneOperation(HookClass):
    ...
    def _createReadNode(self):
        ...
        outputTemplate = tk.templates["scan_ingestion"]
        ...
        for a in listPath:
            ...
            iSeq.append(fields['SEQ'])

        iSeqRef = iSeq[0]
        iSeqLength = len(str(iSeqRef))
        iSeqLengthStr = str(iSeqLength)
        iSeqValue = '%0'+ iSeqLengthStr +'d'
        lengthPath = len(apath)
        lengthNoISeq = lengthPath-iSeqLength
        lengthNoISeqNoExt = lengthNoISeq - 4
        pathName = apath[0:lengthNoISeqNoExt] + iSeqValue + '.dpx'
        ...
        nuke.Root().knob('format').setValue('4K_DCP')
        nuke.Root().knob('first_frame').setValue(1)
        a = read_node['first'].value()
        b = read_node['last'].value()
        c = b - a + 1
        nuke.Root().knob('last_frame').setValue(c)
        message = "script frame range and project aspect ratio are set regarding your scan file," \
                  " Read node is also created based on your scan file," \
                  " your script is saved in project/shot/shotName/nuke/ as v001, please use shotgun tab to save new" \
                  " versions and don't change the script name, don't do please ie: Shot03.v001_myName.nk " \
                  " If you want to work in local please use setReadLocal tool under Lamppost tab"
