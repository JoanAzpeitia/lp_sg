...
class PublishHook(Hook):
            ...
            if output["name"] == "alembic_cache":
                ...
                try:
                   self.__publish_rendered_images(item, output,
                       ...
                       thumbnail_path, progress_cb, task)
                except Exception, e:
                   errors.append("Publish failed - %s" % e)
    ...
    def __publish_rendered_images(self, item, output, work_template,
        primary_publish_path, sg_task, comment, thumbnail_path, progress_cb, task):
        """
        ...
        #copy files from render path to 2D CG folder
        import sgtk
        ...
        for fi, rf in enumerate(render_files):
            ...
            #construct the comp CG files path
            fields = render_template.get_fields(rf)
            fields["Rendered Image"] = tank_type
            comp_CG_template = tk.templates["maya_shot_render_publish_mono_exr"]
            target_path = comp_CG_template.apply_fields(fields)
            ...
            try:
                ...
                raise TankError("Failed to copy file from %s to %s - %s" % (rf, publish_path, e))  # register the publish:
