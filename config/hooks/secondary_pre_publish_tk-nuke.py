    ...
    def _nuke_execute(self, tasks, work_template, progress_cb, **kwargs):
        ...
        for task in tasks:
            ...
            if output["name"] == "render":
                ...
                # get list of render files and get write node:
                write_node = item.get("other_params", dict()).get("node")
                render_files = write_node_app.get_node_render_files(write_node)
                for a_file in render_files:
                    path, ext = os.path.splitext(a_file)
                    if ext != ".dpx":
                        raise TankError("Becareful you are about to send to review a qt that is not a final render"
                                        " (a DPX file) for this please use Shotgun Quick Dailies")
